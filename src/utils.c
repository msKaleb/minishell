...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/07/28 11:41:39 by msoria-j          #+#    #+#             */
/*   Updated: 2023/10/09 18:51:15 by msoria-j         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*create_prompt(t_msh *tm)
{
	...
	if (ft_strcmp(cwd, temp2) == 0)
		pcwd = "~";
	else if (ft_strcmp(cwd, "/") == 0)
		pcwd = "/";
	free(temp2);
	prompt = ft_strdup("\001\x1b[1;36m\002[");
	temp = ft_strdup("USER");
	temp2 = get_env(*tm->tenvp, temp);
	prompt = path_join(prompt, temp2);
	free(temp2);
	prompt = path_join(prompt, "@minishell \001\x1b[1;37m\002");
	prompt = path_join(prompt, pcwd);
	prompt = path_join(prompt, "\001\x1b[1;36m\2]$\001\x1b[0m\002 ");
	return (prompt);
}
